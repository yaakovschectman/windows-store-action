  ...
  toString(): string {
    ...
    if (this.error != undefined) {
      ...
      var statusCode: string =
        this.response != undefined && this.response.statusCode != undefined
    ...
    if (
      ...
      log =
        log + ` CorrelationId: ${this.response.headers["ms-correlationid"]}`;
    }
...
export function performRequest<T>(
  ...
  var callback = function (error: any, response: any, body: any) {
    ...
    if (
      response != undefined && // response is undefined if a transport-level error occurs
  ...
  var expirationCheck = function () {
    if (isExpired(auth)) {
      return authenticate(auth.resource, auth.credentials)
        .then(function (newAuth: { token: string; expiration: number }) {
...
export function authenticate(
  resource: string,
  credentials: Credentials
): Q.Promise<AccessToken> {
  var endpoint =
    "https://login.microsoftonline.com/" + credentials.tenant + "/oauth2/token";
  var requestParams = {
  ...
  return performRequest<any>(options)
    .then<AccessToken>((body: { expires_on: any; access_token: any }) => {
...
export function withRetry<T>(
  ...
  return promiseGenerator().fail((err: any) => {
    if (numRetries > 0 && (!errPredicate || errPredicate(err))) {
      var randomDelay: number = Math.floor(
        Math.random() * RETRY_DELAY + RETRY_DELAY
      ); // RETRY_DELAY <= randomDelay  < 2 * RETRY_DELAY
      console.log(`Operation failed with ${err}`);
      console.log(
        `Waiting ${randomDelay / 1000} seconds then retrying... (${
          numRetries - 1
        } retrie(s) left)`
      );
...
export function isRetryableError(err: any, relax: boolean = true): boolean {
  ...
  if (
    ...
    return (
      err.response.statusCode == 429 || // 429 code is returned by the API for throttle down. This is retriable
...
function logErrorsAndWarnings(response: any, body: any) {
  if (body === undefined || body.statusDetails === undefined) return;

  if (
    Array.isArray(body.statusDetails.errors) &&
    body.statusDetails.errors.length > 0
  ) {
    console.error("Errors occurred in request");
    (<any[]>body.statusDetails.errors).forEach((x) =>
      console.error(`\t[${x.code}]  ${x.details}`)
    );
  }

  if (
    Array.isArray(body.statusDetails.warnings) &&
    body.statusDetails.warnings.length > 0
  ) {
    console.debug("Warnings occurred in request");
    (<any[]>body.statusDetails.warnings).forEach((x) =>
      console.debug(`\t[${x.code}]  ${x.details}`)
    );
  }
